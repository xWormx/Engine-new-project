...
bool Collision_AABB(V2f pa, V2f sa, V2f pb, V2f sb);
bool Collision_AABB(V3f pa, V3f sa, V3f pb, V3f sb);
...
bool Collision_AABB(V2f pa, V2f sa, V2f pb, V2f sb)
{
	bool collided = false;
	float e1_L = pa.x;
	float e1_R = pa.x + sa.x;
	float e1_T = pa.y + sa.y;
	float e1_B = pa.y;

	float e2_L = pb.x;
	float e2_R = pb.x + sb.x;
	float e2_T = pb.y + sb.y;
	float e2_B = pb.y;
