...
struct CJ_CONSOLE
{
	...
	// This string is used when Enter is pressed
	u8 	*str_buffer_to_use;

	// This is the string that's written in the console window
	u8 	*str_base;
	u8	*str_clear_buffer;
	u32	buffer_size;
	i32	char_index;
...
void HandleConsole(CJ_CONSOLE *console, u32 max_console_chars_to_render);
...
void HandleConsole(CJ_CONSOLE *console, u32 max_console_chars_to_render)
{
	BeginConsole(console, max_console_chars_to_render);
...
void BeginConsole(CJ_CONSOLE *console, u32 n_chars_to_allocate)
{
	if(!console->str_base)
	{
		console->max_console_chars_to_render = n_chars_to_allocate;
		...
		console->color			= COLOR_BLACK;
		console->executed		= false;
		console->bkg_window->pos	= console->start_pos;
		console->bkg_window->size	= v2f(2.0f, 2.0f);
		console->bkg_window->col	= console->color;
	}
...
void UpdateConsole(CJ_CONSOLE *console)
{
		// OBS §-knappen till vänster om 1 är i själva verket '`'-symbolen.
		// Så '`' måste användas om man vill öppna konsol med tryck på §.
		if(KeyPressedOnce('`'))
		{
			if(console->mode == cj_console_mode_off)
				console->mode = cj_console_mode_on;
			else
				console->mode = cj_console_mode_off;
		}
		...
		//printf("pos: %0.2f, %0.2f\t", console->bkg_window->pos.x, console->bkg_window->pos.y);
		//printf("size: %0.2f, %0.2f\n", console->bkg_window->size.x, console->bkg_window->size.y);
